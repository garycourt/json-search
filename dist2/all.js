...
if(typeof Array.orderedInsert !== "function") {
  Array.orderedInsert = function(a, b, c) {
    ...
    if(a.length === 0) {
      ...
      e = 0;
      f = a.length - 1;
      for(g = Math.floor(f / 2);f - e > 0;) {
        c(a[g], b) <= 0 ? e = g + 1 : f = g - 1, g = Math.round(e + (f - e) / 2)
      }
...
try {
  ...
  EventEmitter.DEFAULT_MAX_LISTENERS = 10, EventEmitter.prototype.setMaxListeners = function(a) {
    ...
    if(typeof c == "function") {
      ...
      if(Array.isArray(c)) {
        for(var b = Array.prototype.slice.call(arguments, 1), c = c.slice(), e = 0, f = c.length;e < f;e++) {
          c[e].apply(this, b)
        }
        return!0
      }else {
    ...
    var c = this._events[a];
    if(Array.isArray(c)) {
      for(var e = -1, f = 0, g = c.length;f < g;f++) {
...
Stream.prototype.pipe = function(a, b) {
  function c(b) {
    a.writable && !1 === a.write(b) && d.pause()
  }
  function e(b) {
    a.emit("error", b);
    d.destroy()
...
MemoryIndex.prototype.getTermVectors = function() {
};
